  ...
  async generateText(options: GenerateTextOptions): Promise<string> {
    ...
    const data = await withRetry(async () => {
      const response = await fetch(`${this.baseUrl}/api/chat`, {
        ...
        body: JSON.stringify({ model, messages, stream: false, options: { num_predict: 8192 } }),
      });
      if (!response.ok) {
  ...
  async generateWithTools(options: GenerateWithToolsOptions): Promise<LLMToolCallResult> {
    ...
    const data = await withRetry(async () => {
      const response = await fetch(`${this.baseUrl}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model, messages, tools, stream: false, options: { num_predict: 8192 } }),
      });
      if (!response.ok) {
        const body = await response.text();
        throw new Error(`Ollama error ${response.status}: ${body}`);
