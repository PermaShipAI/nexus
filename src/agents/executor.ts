      ...
      while ((match = approveRegex.exec(cleaned)) !== null) {
        try {
          ...
          if (action.status === 'waiting_for_human') {
            ...
            cleaned += `\n\n[SYSTEM] Action Blocked: Ticket ${parsed.id} is locked in 'waiting_for_human' pending mandatory manual approval. Do not attempt to retry this action.`;
            continue;
          }
      ...
      while ((match = missionCompleteRegex.exec(cleaned)) !== null) {
        try {
          ...
          if (completedItem.status === 'waiting_for_human') {
            ...
            cleaned += `\n\n[SYSTEM] Action Blocked: Mission item ${parsed.itemId} is locked in 'waiting_for_human' pending mandatory manual approval. Do not attempt to retry this action.`;
            continue;
          }
