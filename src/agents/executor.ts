    ...
    if (!input.isStrictConsultation) {
      ...
      while ((match = missionAddItemsRegex.exec(cleaned)) !== null) {
        try {
          ...
          logger.warn({ err, agentId }, 'Failed to parse/process mission-add-items block');
        }
      }
      ...
      cleaned = cleaned.replace(/<mission-replace-item>\s*[\s\S]*?(?:<\/mission-replace-item>|$)/gi, '').trim();

      // Extract and process <mission-remove-item> blocks (Nexus only)
      const missionRemoveRegex = /<mission-remove-item>\s*([\s\S]*?)(?:<\/mission-remove-item>|$)/gi;
      while ((match = missionRemoveRegex.exec(cleaned)) !== null) {
        try {
          const parsed = JSON.parse(match[1].trim()) as { itemId: string; reason: string };
          if (!parsed.itemId) continue;
...
function suppressProposalDetails(agentId: string, response: string, proposalCount: number): string | null {
  ...
  if (trimmed.length === 0) {
    if (proposalCount > 0) {
      // Agent's entire response was XML action blocks — surface a brief actionable summary
      logger.info({ agentId, proposalCount }, 'Agent response was empty after XML stripping; returning proposal summary');
      const noun = proposalCount === 1 ? 'proposal' : 'proposals';
      return `I've queued ${proposalCount} task ${noun} for review.`;
    }
    // Nothing to say — suppress the empty message entirely
    logger.info({ agentId }, 'Agent response was empty after XML stripping; suppressing');
    return null;
  }
