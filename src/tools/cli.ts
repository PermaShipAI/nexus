...
import { createTask, updateTaskStatus, listTasks, StateMachineConflictError } from '../tasks/service.js';
  ...
  switch (command) {
    ...
    case 'update-task': {
      ...
      try {
        ...
        if (err instanceof StateMachineConflictError) {
          printResult({ success: false, status: 'blocked', reason: err.message, ACTION_REQUIRED: 'Acknowledge failure and halt retry.' });
        } else {
