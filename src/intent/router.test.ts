...
describe('routeIntent', () => {
  ...
  it('requires confirmation for AdministrativeAction (enable autonomous mode)', async () => {
  ...
  it('blocks low-confidence AdministrativeAction for ambiguous settings request', async () => {
    const result = await routeIntent('change the setting', privateAdminContext);
    expect(result.allowed).toBe(false);
    expect(result.blockReason).toBe('LowConfidence');
  });

  it('blocks MEMBER from AdministrativeAction (requires ADMIN)', async () => {
